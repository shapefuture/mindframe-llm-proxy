...
const CognitiveMirrorStep: React.FC<CognitiveMirrorStepProps> = ({ profile, isLoading }) => {
  React.useEffect(() => {
    console.log('[CognitiveMirrorStep] Mounted with props:', { profile, isLoading });
  }, [profile, isLoading]);
  ...
  try {
    ...
    if (!profile) {
      console.warn('[CognitiveMirrorStep] No profile to display');
    ...
    console.error('[CognitiveMirrorStep] Render error:', error);
    return (
      <div className="p-4 text-red-600 bg-red-100 rounded">
        <h2 className="text-lg font-bold">Error</h2>
        <p>There was a problem displaying your cognitive snapshot. Please reload or contact support.</p>
      </div>
    );
  }
};

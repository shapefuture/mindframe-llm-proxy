...
const HCRatingStep: React.FC<HCRatingStepProps> = ({ hcItems, hcProficiency, onHCProficiencyChange }) => {
  React.useEffect(() => {
    console.log('[HCRatingStep] Mounted with props:', { hcItems, hcProficiency });
  }, [hcItems, hcProficiency]);
  ...
  try {
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  console.log('[HCRatingStep] User changed proficiency:', { hcId: hc.id, value: val });
                  onHCProficiencyChange(hc.id, val);
                }}
    ...
    console.error('[HCRatingStep] Render error:', error);
    return (
      <div className="p-4 text-red-600 bg-red-100 rounded">
        <h2 className="text-lg font-bold">Error</h2>
        <p>There was a problem displaying the cognitive skills rating step. Please reload or contact support.</p>
      </div>
    );
  }
};

...
const WelcomeStep: React.FC<WelcomeStepProps> = ({ onInterestsChange, selectedInterests, interestOptions }) => {
  React.useEffect(() => {
    console.log('[WelcomeStep] Mounted with props:', { selectedInterests, interestOptions });
  }, [selectedInterests, interestOptions]);
  ...
  try {
            ...
            {interestOptions.map(interest => (
              <label key={interest.id} className={`flex items-center space-x-2 p-2 border rounded-md hover:bg-secondary/50 transition-colors cursor-pointer ${selectedInterests.includes(interest.id) ? 'bg-primary/10 ring-1 ring-primary' : ''}`}>
                <input
                  ...
                  onChange={(e) => {
                    console.log('[WelcomeStep] User toggled interest:', { interestId: interest.id, checked: e.target.checked });
                    onInterestsChange(interest.id, e.target.checked);
                  }}
                />
    ...
    console.error('[WelcomeStep] Render error:', error);
    return (
      <div className="p-4 text-red-600 bg-red-100 rounded">
        <h2 className="text-lg font-bold">Error</h2>
        <p>There was a problem displaying the welcome step. Please reload or contact support.</p>
      </div>
    );
  }
};
